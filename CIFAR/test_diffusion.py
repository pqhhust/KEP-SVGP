...
def process_results(args, loader, model, metrics, logger, method_name, results_storage, pretrained_ViT):
    res = valid_diffusion.validation(loader, model, args, pretrained_ViT)
    for metric in metrics:
        results_storage[metric].append(res[metric])
    log = [f"{key}: {res[key]:.3f}" for key in res]
    logger.info(f'################## \n ---> Test {method_name} results：\t' + '\t'.join(log))

def test_cifar_c_corruptions(dataset, model, corruption_dir, transform_test, batch_size, metrics, logger, pretrained_ViT):
    if dataset == "cifar10":
        ...
        for corruption in data.CIFARC.CIFAR10C.cifarc_subsets:
            for severity in range(1, 6):
                ...
                res = valid_diffusion.validation(corrupted_test_loader, model, args, pretrained_ViT)
                for metric in metrics:
...
def test():
    ...
    for r in range(args.nb_run):
        ...
        pretrained_ViT.load_state_dict(torch.load(os.path.join(save_path, f'best_acc_net_{r + 1}.pth')))
        pretrained_ViT.cuda()
        net.load_state_dict(torch.load(os.path.join(save_path, f'best_acc_net_{r + 1}_diffusion.pth')))
        net = net.cuda()
